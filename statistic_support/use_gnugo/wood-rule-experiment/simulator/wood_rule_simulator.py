...
class WoodGoSimulator:
    def __init__(self, gnugo_path="gnugo"):
    ...
    def get_random_corner_position(self, board_size=19):
        ...
        # 选择角落位置的列（左/右边界）
        col_choices = ['j']  # b10       左边界a/b，右边界s/t
        corner_col = random.choice(col_choices)
        
        # 选择行（1路或2路）
        corner_row = 'b' if row == 1 else 'b'
...
;W[{white_stone_pos}])"""
        ...
        try:
            ...
            # 启动GNUGO进程（Linux版本，移除Windows特定标志）
            ...
            if any(keyword in stdout_lower for keyword in ["black wins", "b+", "black+"]):
                ...
                # 规则规定：子/目相同时 black 胜
                result = "black"
                print(f"DEBUG: 第{game_id+1}局未找到明确结果，判定为黑胜")
            ...
            try:
                ...
                print(f"DEBUG: 尝试使用简化命令: {' '.join(simple_cmd)}")
                ...
                simple_stdout, simple_stderr = simple_process.communicate(timeout=15)
                print(f"DEBUG: 简化命令输出: {simple_stdout[:100]}...")
                if "black" in simple_stdout.lower():
                    result = "black"
                    print(f"DEBUG: 简化命令判定为黑胜")
            except:
                pass
            # 尝试使用更简单的命令
            try:
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='Wood围棋规则Linux测试')
    parser.add_argument('-n', '--games', type=int, default=5, help='模拟对局数 (默认: 5)')
    parser.add_argument('--gnugo-path', type=str, default="gnugo", help='GNUGO可执行文件路径 (默认: gnugo)')
